...
NMEA_PAYLOADS_GET = {
    ...
    "MARIMU": {
        ...
        "pitch": DE,
        "yaw": DE,
        "p": DE,  # roll rate of change
        "q": DE,  # pitch rate of change
        "r": DE,  # yaw rate of change
    },
    # Motor status
    "MAROTR": {
        "rpm_port": DE, # RPM (always positive) (port side)
        "rpm_stb": DE, # RPM (always positive) (starboard side)
        "temp_port": DE, # temperature (deg C) (port side)
        "temp_stb": DE, # temperature (deg C) (starboard side)
        "power_port": DE,  # power (W) (port side)
        "power_stb": DE,  # power (W) (starboard side)
        "power_total": DE,  # total power (W)
        "batteries_count": IN, # number of batteries
    },
    "MARMOD": {
        "mode": CH,  # current mode (e.g. MAN, CRS, etc.)
        "fuel_capacity": DE,  # fuel capacity (range [0,1])
    },
    "MARERR": {
        "description": CH,  # error description
    },
}
